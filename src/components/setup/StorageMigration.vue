...
import {
  unifiedStorageMigrationService,
  type UnifiedMigrationResult,
  type MigrationSource,
  isChromeExtension,
} from '@/services/unifiedStorageMigration'
...
const migrationResult = ref<UnifiedMigrationResult | null>(null)
const migrationProgress = ref<MigrationProgress | null>(null)
const showDetails = ref(false)
const isExtension = ref(isChromeExtension())
...
const stageLabel = computed(() => {
  ...
  const source = migrationProgress.value.source
  const sourceLabel = source === 'chromeStorage' ? ' (Extension)' : source === 'localStorage' ? ' (Web)' : ''
  ...
  return (labels[stage] || stage) + sourceLabel
})

const hasPendingMigration = computed(() => migrationStatus.value.needsMigration)
const migrationCompleted = computed(() => 
  migrationStatus.value.localStorage === 'completed' && 
  (!isExtension.value || migrationStatus.value.chromeStorage === 'completed')
)

const pendingStorageTypes = computed(() => {
  const types: string[] = []
  if (migrationStatus.value.localStorage === 'pending') {
    types.push('Web Storage (localStorage)')
  }
  if (migrationStatus.value.chromeStorage === 'pending') {
    types.push('Extension Storage (chrome.storage)')
  }
  return types
})
...
const checkMigrationStatus = async () => {
  try {
    migrationStatus.value = await unifiedStorageMigrationService.getMigrationStatus()
    
    if (migrationCompleted.value) {
      console.log('[StorageMigration] Migration already completed')
    } else if (hasPendingMigration.value) {
      console.log('[StorageMigration] Pending migrations:', {
        localStorage: migrationStatus.value.localStorage,
        chromeStorage: migrationStatus.value.chromeStorage,
      })
    }
  } catch (error) {
...
const startMigration = async () => {
  ...
  try {
    const result = await unifiedStorageMigrationService.executeUnifiedMigration((progress) => {
    ...
    if (result.success) {
      migrationStatus.value = {
        ...
        chromeStorage: isExtension.value ? 'completed' : 'none',
        needsMigration: false,
      }
      setTimeout(() => {
...
<template>
  <div class="storage-migration">
    ...
    <UiCard v-if="hasPendingMigration && !isMigrating && !migrationResult" class="migration-notice">
      ...
      <div v-if="pendingStorageTypes.length > 0" class="notice-sources">
        <h4 class="sources-title">Data sources to migrate:</h4>
        <ul class="sources-list">
          <li v-for="type in pendingStorageTypes" :key="type">📦 {{ type }}</li>
        </ul>
      </div>
    ...
    <UiCard v-if="migrationResult && !isMigrating" class="migration-result">
      ...
      <div class="result-stats">
        ...
        <template v-if="migrationResult.localStorage">
          <div class="stats-section">
            <h4 class="stats-section-title">Web Storage (localStorage)</h4>
            <div class="stat-item">
              <span class="stat-label">Stream Sources:</span>
              <span class="stat-value">{{ migrationResult.localStorage.migratedSources }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Media Items:</span>
              <span class="stat-value">{{ migrationResult.localStorage.migratedMediaItems }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Favorites:</span>
              <span class="stat-value">{{ migrationResult.localStorage.migratedFavorites }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Watch History:</span>
              <span class="stat-value">{{ migrationResult.localStorage.migratedRecents }}</span>
            </div>
          </div>
        </template>

        <!-- chrome.storage Results -->
        <template v-if="migrationResult.chromeStorage">
          <div class="stats-section">
            <h4 class="stats-section-title">Extension Storage (chrome.storage)</h4>
            <div class="stat-item">
              <span class="stat-label">Stream Sources:</span>
              <span class="stat-value">{{ migrationResult.chromeStorage.migratedSources }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Media Items:</span>
              <span class="stat-value">{{ migrationResult.chromeStorage.migratedMediaItems }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Favorites:</span>
              <span class="stat-value">{{ migrationResult.chromeStorage.migratedFavorites }}</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">Watch History:</span>
              <span class="stat-value">{{ migrationResult.chromeStorage.migratedRecents }}</span>
            </div>
          </div>
        </template>
      </div>
